...
- **Список СИ** (главная) — сортировка по инв. номеру, статусу, датам поверки, году выпуска; фильтры (тип, статус); поиск; экспорт XLSX; сохранение состояния (сортировка, фильтры, пагинация) при переходах
- **Карточка СИ** — основные данные (включая год выпуска), поверки (пред./след. дата), ремонты, документы, история изменений
- **Автоприсвоение инв. номера** при создании
- **Импорт XLSX** — вкладка в настройках: загрузка → маппинг колонок с пагинацией по колонкам → предпросмотр → коммит; автодетект заголовков; корректный парсинг Excel serial dates (например, 45441 → YYYY-MM-DD)
...
- **Cron-задачи** — автоматический пересчёт статусов поверок (ACTIVE, VERIFICATION_MONTH, VERIFICATION_14DAYS, EXPIRED) на основе даты следующей поверки
