...
export class ImportController {
  ...
  async upload(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const { importId, headers } = await service.upload(req.file.path, mapping);
      res.json({ success: true, import_id: importId, headers });
    } catch (err) { next(err); }
  }
  ...
  async updateMapping(req: Request, res: Response, next: NextFunction) {
    try {
      const ok = service.updateMapping(req.params.id as string, req.body.mapping || {});
      if (!ok) {
